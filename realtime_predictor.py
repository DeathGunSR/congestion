    ...
    try:
        ...
        while True:
            ...
            if len(data_queue) == SEQUENCE_LENGTH:
                ...
                color = 'red' if status == "Worsening" else 'green'

                # Shade the background of the last interval
                if len(time_steps) > 1:
                    ax.axvspan(time_steps[-2], time_steps[-1], facecolor=color, alpha=0.3)
            ...
            ax.set_title(f"Real-Time RTT and Congestion Prediction (Activity: {args.activity})")
            ax.legend()
            ax.grid(True)
            fig.canvas.draw()
            plt.pause(0.01)
